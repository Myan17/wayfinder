...
owner: myan
paths:
  - apps/api/wayfinder/http/**
  - docs/context/modules/http.md
interface_files:
  - openapi.json
  - docs/api/errors.md
tables_owned:
...
depends_on:
  ...
  - cache
design_sections:
  - "DESIGN §9.9 (API surface and SSE)"
  - "DESIGN §13.2 (session, CSRF, admin authority)"
verified_at: 0000000
...
> **Whoever implements `http` fills it in, in the pull request that lands the first interface file**
...
Routes, authentication flows, SSE transport, admission control and the error contract.
