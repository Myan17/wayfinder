...
verified_hashes:
  "db/schema.sql": "302e84b53272e328"
...
> **Status (2026-09-23):** v1 is four migrations: `_index`, `_principals`, `_views`, `_deletion`. Tests
> run against the S3 ParadeDB digest with `WAYFINDER_TEST_DSN` set, and skip without it. `db/schema.sql`
> is generated by `db/dump-schema.sh` (`--check` reports drift); CI runs the check from ORIENT item 3.
...
| 2026-09-23 | `db/schema.sql` snapshot and `db/dump-schema.sh` generator | — |
