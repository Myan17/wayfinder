...
- Keep the branch small: **≤ 3 days and ≤ 400 changed lines** (added plus deleted). Bigger than
  ...
  - **Two exceptions, both exact paths:**
    - The task's own `docs/agent-log/<task>.md` (agreed 2026-09-20). It is process output that grows
      with every commit, and counting it pushed authors to trim code comments to make room for it —
      the rule pressuring the wrong thing.
    - The repository-root `db/schema.sql` (reviewer ruling, 2026-09-23). It is the full canonical
      `pg_dump --schema-only` output from `db/dump-schema.sh`, reviewed **mechanically**: the reviewer
      reads the generator, and a drift check proves the committed file equals a fresh dump. No other
      path is exempt, including files named like it anywhere else.
  - Source, tests, ordinary documentation, configuration and **every other generated file count**. A
    generated file large enough to matter goes in its own pull request, which is a review decision,
    not an accounting trick. An exemption is never introduced by the pull request that benefits
    from it.
...
| `size` | More than 400 changed lines, excluding the task's own log and the exact path `db/schema.sql` (`scripts/check_pr_size.py`) |
